...
router.post("/place-bid", authMiddleware, async (req, res) => {
  ...
  try {
    const existbid = bidPrice.findOne({ productId, userId });
    if (existbid) {
      return res
        .status(400)
        .json({ msg: "You have already placed a bid for this product." });
    }
...
async function handleBidEnd(productId) {
  try {
    const bids = await bidPrice.find({ productId }).sort({ price: -1 });
...
setInterval(async () => {
  try {
    ...
    for (const product of products) {
      // Schedule email notifications 1 minute after bid end
      setTimeout(() => handleBidEnd(product._id), 60 * 1000);
    }
  } catch (error) {
